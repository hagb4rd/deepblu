...
var Vector2D = {
    ...
    add(...v) {
        ...
        return Vector2D.create(v.map(v_=>v_.x).reduce((prev,next)=>prev+next),v.map(v_=>v_.y).reduce((prev,next)=>prev+next)) 
    },
    toJSON() {
...
function d6() { var r=Math.floor(Math.random()*6)+1; console.log(r); return r; }; var dice = new (require('events').EventEmitter)(); function promise(number) { var trials=3; return new Promise((resolve, reject) => { dice.on('roll', function(e) { if(trials--) reject('no luck this time'); if(e===number) resolve(e); }); }); promise(4).then(s=>console.log(s + " received!")).catch(console.log); dice.emit('roll', d6());
...
var inherits = function inherits(base) { if (base.constructor == Function) { /* inherit */ this.prototype = new base; this.prototype.constructor = this; this.prototype.base = base.prototype; } else { /* virtual */ this.prototype = base; this.prototype.constructor = this; this.prototype.base = base; } return this; }; Function.prototype.inherits = inherits;
...
var range = function range(a, b) {
    for (var r = []; r.length <= b - a; r.push(a + r.length));
    return r;
};
exports.range = range;
var timer = function timer(f) { var t1 = Date.now(); f(); var t2 = Date.now(); return (t2 - t1); };
exports.timer = timer;
var stats = function stats(f, n) { n = n || 5; var a = range(0, n); return a.map(x=> timer(t=> f())); };
exports.stats = stats;
...
var map = function map(arr, f) { var result = new Array(arr.length); for(var i=arr.length;i-->0;) result[i]=f(arr[i]); return result; }; 
exports.map = map; 
...
exports.leaves = function* leaves(tree) {
    for (var key of Object.keys(tree)) {
        if (typeof tree[key] === 'object' && tree[key] !== null) {
            yield * leaves(tree[key]);
        } else {
            yield tree[key];
        }
    }
